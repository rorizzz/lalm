"""QA evaluation / decoding script for lalm (Qwen3, no-thinking).

Same as evaluate_qa.py, but splits the prompt at Qwen3's no-thinking assistant
prefix ('<|im_start|>assistant\\n<think>\\n\\n</think>\\n\\n') so the pre-filled
empty think block is kept in the prompt. Assumes test manifests were prepared
by prepare_conversation_qa_qwen3.py.

Usage::

    python evaluate_qa_qwen3.py \
        exp_dir=/path/to/exp \
...
# ---------------------------------------------------------------------------
# Model loading  (reused from evaluate_qa.py)
# ---------------------------------------------------------------------------
...
def _resolve_checkpoint_path(cfg: DictConfig) -> tuple[str, str]:
    ...
    raise ValueError(
        "[evaluate_qa_qwen3] Specify checkpoint.filename, checkpoint.iter, or checkpoint.epoch."
    )
...
def prepare_model_dir(cfg: DictConfig) -> tuple[str, str]:
    ...
    if _has_hf_weights(model_dir):
        logging.info(f"[evaluate_qa_qwen3] Using existing model dir: {model_dir}")
        return model_dir, results_suffix

    logging.info(f"[evaluate_qa_qwen3] Exporting {checkpoint_path} -> {model_dir}")
    hf_dir = os.path.join(cfg.exp_dir, "hf")
    if not os.path.isdir(hf_dir):
        raise FileNotFoundError(
            f"[evaluate_qa_qwen3] hf_dir not found: {hf_dir}. Run train.py first."
        )
    ...
    logging.info(f"[evaluate_qa_qwen3] Saved to {model_dir}")
...
# Qwen3 no-thinking assistant prefix: keep the empty think block in the prompt
# so the frozen LM starts generating right after '</think>\n\n'.
...
def main(cfg: DictConfig):
    ...
    logging.info(
        f"[evaluate_qa_qwen3] {sum(p.numel() for p in model.parameters()) / 1e6:.1f}M params"
    )
    ...
    for test_set in test_data_config:
        logging.info(f"[evaluate_qa_qwen3] Test set: {test_set['name']}")
        ...
        logging.info(f"[evaluate_qa_qwen3] Results saved to {results_path}")
        ...
        logging.info(
            f"[evaluate_qa_qwen3] {test_set['name']} Accuracy: {acc:.2f}% ({correct}/{total})"
        )
